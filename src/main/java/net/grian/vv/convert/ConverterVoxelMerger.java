...
public class ConverterVoxelMerger implements Converter<VoxelArray, VoxelMesh> {
    ...
    public VoxelMesh invoke(VoxelArray array, Object... args) {
        logger.info("merging "+array+" ...");
        final int
                limX = array.getSizeX(),
                limY = array.getSizeY(),
                limZ = array.getSizeZ();

        LineList[][] lines = mergeX(array, limX, limY, limZ);
        {
    ...
    public ConverterVoxelMerger() {
        this.logger = Logger.getGlobal();
    }

    /**
     * Merges voxels into lines of voxels on the x-axis.
     *
     * @param voxels the initial voxel array
     * @return a 2D array of voxel arrays lists representing a YxZ array of lines
     */
    private static LineList[][] mergeX(VoxelArray voxels, int limX, int limY, int limZ) {
    ...
    private static VoxelMesh cut(VoxelArray array, BoxList boxes) {
        VoxelMesh result = new VoxelMesh();

        for (BoxList.Entry box : boxes) {
            VoxelArray copy = array.copy(box.xmin, box.ymin, box.zmin, box.xmax, box.ymax, box.zmax);
            result.add(box.xmin, box.ymin, box.zmin, copy);
        }
